...
class AddRecipeUI:
    def __init__(self, root, result_label, parent_recipe_app):
        ...
        # add a notes label
        notes_label = ttk.Label(root, text="Leave a note for yourself")
        notes_label.pack(pady=5)
        self.notes_box = tk.Text(root, wrap=tk.WORD, width=50, height=15)
    ...
    def add_recipe(self) -> None:
        '''Creates a DatabaseInsertFetch object to add a recipe to the database. If an error message is returned, display it in the result label. Otherwise, confirm successful insertion in the result label.'''
        type_ = self.combo_box.get()
        name = self.entry_recipe.get()
        link = self.entry_link.get()
        notes = self.notes_box.get("1.0", tk.END)

        with DatabaseInsertFetch("add", type_, name, link, notes) as db_obj:
            error = db_obj.insert_recipe()  # returns a string if there is an error. else, None is returned
            if error:
                self.result_label.config(text=error)
            else:
                self.result_label.config(text=f"Recipe '{name.strip()}' added to database!")
