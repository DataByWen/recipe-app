...
class DatabaseInsertFetch():
    ...
    def __init__(self, action: str, combobox_choice: str, entry_name, entry_link, notes_box):
    ...
    def _transform(self) -> None:
        """Removes leading and trailing whitespaces when you add in a recipe. Also, converts all empty strings to NULLs."""
        if self.action == "add":
            self.entry_name = self.entry_name.strip()
            self.entry_link = self.entry_link.strip()
            self.notes_box = self.notes_box.strip()
            
            # convert empty strings to NULLs
            values = [self.entry_name, self.entry_link, self.combobox_choice, self.notes_box]
            values = [None if v.strip() == "" else v for v in values]
            self.entry_name, self.entry_link, self.combobox_choice, self.notes_box = values
    ...
    def insert_recipe(self) -> Union[str, None]:
        ...
        # if the entry name is already in the db, we return an error message
        query = "SELECT 1 FROM recipes WHERE name = %s LIMIT 1"
        values = (self.entry_name,) # needs to be a list, tuple, or dict
        ...
        if result:
            ...
            query = "INSERT INTO recipes (name, type, link, notes) VALUES (%s, %s, %s, %s)"
            values = (self.entry_name, self.combobox_choice, self.entry_link, self.notes_box)
            self.cursor.execute(query, values)
            self.connection.commit()
            return None

    def fetch_recipe(self) -> tuple:
        """Fetches a recipe from the database based on the recipe type selected by the user and returns a tuple: (name, type, link, notes)."""
        # check if the table contains rows/recipes first
        self.cursor = self.connection.cursor()
        self.cursor.execute("SELECT COUNT(*) FROM recipes;") 
        row_count = self.cursor.fetchone()[0]
        if row_count == 0:
            return (None, None, None, None) # name, type, link, notes all empty
        
        if (self.combobox_choice == "Any") or (self.combobox_choice == ""): 
            self.cursor.execute(
                """
                SELECT name, type, link, notes 
                FROM recipes
                ORDER BY RAND()
                LIMIT 1;
                """
